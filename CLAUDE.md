...
**Organization**: 7D Solutions
**Purpose**: Generic AI assistant context for all 7D Solutions projects
**Working directory**: `/mnt/c/Users/7d.vision/Projects/7D Solutions/`
...
**Standard Structure**:
```
/backend/          → Node.js API (Express) - Port 8000
/frontend/         → React app (Vite) - Port 3001
/database/         → Database schemas and migrations
```

**Alternative Structure (with shared services layer)**:
```
/backend/          → Node.js API (Express) - Port 8000
/frontend/         → React app (Vite) - Port 3001
/shared/           → Shared services & business logic (project-specific)
```
...
# View logs
docker logs [project]-backend -f
docker logs [project]-frontend -f
...
### Service Details
- **Backend**: Port 8000, Node.js with auto-restart (--watch)
- **Frontend**: Port 3001, Vite HMR (instant .jsx/.tsx updates)
- **Database**: Check project-specific configuration
...
- ❌ **Manual restart needed**:
  ...
  - Infrastructure/shared service changes
...
### Connection Details
- **Type**: MySQL/PostgreSQL (check project-specific configuration)
- **Host**: Common values:
  - `localhost` (from host machine)
  - `host.docker.internal` (from Docker containers)
  - `mysql.railway.internal` (Railway internal network)
- **Port**: Common defaults:
  - MySQL: 3306 (standard) or 3307 (alternate)
  - PostgreSQL: 5432
- **Database**: Check project `.env` file
