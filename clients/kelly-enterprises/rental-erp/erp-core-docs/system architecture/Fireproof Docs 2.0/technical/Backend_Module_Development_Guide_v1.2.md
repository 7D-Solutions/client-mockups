# Backend Module Development Guide v1.2

**Version:** 1.2  
**Date:** 2025-01-15  
**Purpose:** Comprehensive guide for creating secure backend modules following gold standard patterns with enhanced security requirements
...
**CRITICAL**: Pay special attention to sections marked with 🚨 - these address security vulnerabilities that have been exploited in production.
</claude-instructions>
...
4. [🚨 Critical Security Requirements](#critical-security-requirements)
...
- **Parameterized queries ONLY** - no string interpolation for ANY SQL values
...
mkdir -p tests/security/[modulename]  # REQUIRED for security tests
```
...
mkdir -p tests/security/inventory
```
...
### 8. Create Tests (Including Security Tests)

See [Testing Requirements](#testing-requirements) section for complete test templates.

---

## 🚨 Critical Security Requirements

### LIMIT/OFFSET SQL Injection Prevention

**THE VULNERABILITY**: The following pattern creates a SQL injection vulnerability:
...
async findPaginated(filters = {}, page = 1, limit = 10, conn) {
  // CRITICAL: Validate and sanitize pagination inputs
...
**CRITICAL CORRECTION**: MySQL2 DOES support parameterized LIMIT/OFFSET queries. Any code comment stating otherwise is incorrect and dangerous.
...
### Security Validation in BaseRepository

BaseRepository must include query validation:
...
4. **ALWAYS** validate and sanitize input:
   ...
   - Set maximum limits: `Math.min(limit, 1000)`
...
1. **AUDIT** all code for LIMIT/OFFSET template literals:
   ```bash
   grep -r "LIMIT.*\${" src/
   grep -r "OFFSET.*\${" src/
   ```
2. **FIX** all vulnerable queries immediately
3. **TEST** with malicious inputs
4. **VERIFY** test coverage meets minimums
...
### Security Test Template (REQUIRED)
...
describe('[ModuleName]Repository Security', () => {
  ...
  describe('SQL Injection Prevention', () => {
    ...
    test('should detect template literal LIMIT/OFFSET', () => {
      ...
      if (typeof repo.validateQuery === 'function') {
...
Here's a complete inventory module implementation:
...
class InventoryService extends BaseService {
  ...
  async getItemsPaginated(page, limit, filters) {
...
router.get('/',
  ...
  async (req, res, next) => {
    try {
      ...
      const results = await inventoryService.getItemsPaginated(page, limit, filters);
      res.json(results);
    } catch (error) {
      next(error);
    }
...
- [ ] Create directory structure (including security test folder)
- [ ] Create repository extending BaseRepository
- [ ] Implement secure pagination with parameterized LIMIT/OFFSET
...
- [ ] Run security audit: `grep -r "LIMIT.*\${" src/ && grep -r "OFFSET.*\${" src/`

---

## Version History
- v1.0 - Initial gold standard patterns documentation
- v1.1 - Added LIMIT/OFFSET injection prevention, mandatory security tests
- v1.2 - Consolidated security addendum, added test coverage requirements, enhanced security patterns