# Database Complete Reference v2.0
...
3. [Database Schema](#database-schema)
   - 3.1 [Architecture Decisions](#architecture-decisions)
   - 3.2 [Main Gauges Table Modifications](#main-gauges-table-modifications)
   - 3.3 [Thread Gauge Specifications](#thread-gauge-specifications)
   - 3.4 [Hand Tool Specifications](#hand-tool-specifications)
   - 3.5 [Large Equipment Specifications](#large-equipment-specifications)
   - 3.6 [Calibration Standard Specifications](#calibration-standard-specifications)
   - 3.7 [Supporting Tables](#supporting-tables)
   - 3.8 [Common Queries](#common-queries)
4. [Design Decisions](#design-decisions)
   - 4.1 [NULL Column Optimization](#null-column-optimization)
   - 4.2 [Status Change Tracking Pattern](#status-change-tracking-pattern)
   - 4.3 [Event System Design](#event-system-design)
   - 4.4 [Soft Delete Strategy](#soft-delete-strategy)
   - 4.5 [Module Separation Principles](#module-separation-principles)
   - 4.6 [Table Naming Conventions](#table-naming-conventions)
5. [Migration Guide](#migration-guide)
   - 5.1 [Pre-Migration Checklist](#pre-migration-checklist)
   - 5.2 [Migration Phases](#migration-phases)
   - 5.3 [Rollback Plan](#rollback-plan)
   - 5.4 [Post-Migration Validation](#post-migration-validation)
...
- **Normalized Structure**: Proper 3NF normalization with type-specific tables
- **No Wasted Storage**: Eliminates 60% storage waste from NULL columns
- **Extensibility**: Easy to add new equipment types without schema changes
...
|---------|------|---------|---------|
| 2.0 | 2025-09-05 | Consolidated schema, design decisions, and migration guide | Database Specialist |
| 1.0 | 2025-08-18 | Initial normalized schema design | Database Specialist |
...
### Main Gauges Table Modifications

**Note**: This shows modifications to add to existing gauges table
...
- ✗ `core_user_roles.expires_at` - No temporary role assignments
- ✗ `core_user_permission_overrides.expires_at` - Permission overrides don't expire
- ✗ `gauge_items.manufacturer, model_number, purchase_date, purchase_price` - Belongs in future purchasing module
- ✗ `gauge_transfers.approval_notes` - Notes rarely used when accepting/rejecting
- ✗ `gauge_calibrations.environmental_conditions` - Not needed for gauge tracking
- ✗ `gauge_current_state` (entire approach) - Replace with gauge_active_checkouts

#### Kept (Legitimate NULLs)
- ✓ `core_users.locked_until` - Performance checked on every login
- ✓ `core_events.target_module_id, entity_type, entity_id` - Not all events have targets
- ✓ `gauge_items.custom_id` - Customer choice
- ✓ `gauge_items.companion_gauge_id` - Only thread gauges have companions
- ✓ `gauge_calibrations.document_path` - Document upload optional
- ✓ `core_notifications.read_at, expires_at` - NULL until read
- ✓ `gauge_transactions.related_user_id, job_number, location, notes` - Optional details
- ✓ `core_audit_log.user_id, ip_address, user_agent` - System actions lack context

### Status Change Tracking Pattern
...
**Table Prefixes**:
- Core Tables: `core_*` (users, roles, permissions, events, notifications, audit_log)
- Module Tables: `[module]_*` (gauge_items, inventory_stock, quality_inspections)
...
**Special Cases**:
- `gauge_current_state` → `gauge_active_checkouts` (Only track active checkouts)
...
- [ ] Review this complete schema thoroughly
...
Create core tables if not existing (refer to complete definitions above).

#### Phase 2: Gauge Standardization Tables

1. Create gauge_categories table and populate
2. Create all 4 specification tables
3. Create supporting tables (id_config, companion_history, system_config)
...
**3.2 Migrate Specifications**
```sql
-- Thread gauges
INSERT INTO gauge_thread_specifications (gauge_id, thread_size, thread_type, ...)
SELECT id, ... FROM gauges WHERE equipment_category = 'thread_gauge';

-- Similar for other types
```

**3.3 Generate System IDs**
- Use gauge_id_config to generate sequential IDs
- Update gauges with system_gauge_id

**3.4 Identify Companions**
- Match GO/NO GO pairs based on naming patterns
- Update companion_gauge_id fields
...
SELECT g.id FROM gauges g
LEFT JOIN ... (all spec tables)
WHERE specifications missing;
```

### Rollback Plan

1. **Immediate Rollback**: Drop new tables, restore from backup
2. **Partial Rollback**: Keep structure but clear migrated data

### Post-Migration Validation

1. Verify record counts by equipment_category
2. Check specification table populations
3. Test application functionality
4. Validate reporting accuracy

**Migration Timeline**: 1-2 days including testing
...
*This complete reference represents the authoritative database design for the gauge standardization system, implementing clean separation of concerns with type-specific specification tables.*